...
export default function AdminQuotationQueue() {
  ...
  return (
    <div className="space-y-6" data-testid="admin-quotation-queue">
      ...
      {displayedProjects.length === 0 && <p className="bg-white border border-[#E8E4D9] p-6 text-center text-[#333333]">No projects in this stage.</p>}
              ...
              <div className="mt-4 border-t border-[#E8E4D9] pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {p.quotation.milestones.map((ms) => (
                    <div key={ms.id} className={`p-3 border text-xs ${ms.status === 'paid' ? 'bg-[#F3F0E9] border-[#0C1D42]' : ms.status === 'unlocked' ? 'bg-white border-[#DA9E3E]' : 'bg-gray-50 border-gray-200 opacity-60'}`}>
        ...
        <form onSubmit={submit} className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" required value={totalAmount} onChange={e => setTotalAmount(e.target.value)} placeholder="e.g. 1500000" className="w-full p-2 border border-[#E8E4D9] focus:outline-none focus:border-[#0C1D42] text-sm" />
            </div>
            <div>
              <label className="block text-xs uppercase tracking-widest font-bold text-[#0C1D42] mb-2">Quotation PDF File *</label>
              <input type="file" required accept=".pdf" onChange={e => setFile(e.target.files[0])} className="w-full p-2 border border-[#E8E4D9] text-sm file:mr-4 file:py-1 file:px-3 file:border-0 file:text-xs file:bg-[#F3F0E9] file:text-[#0C1D42]" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {milestones.map((m, i) => (
                <div key={i} className="flex gap-2 items-start border border-[#E8E4D9] p-3 bg-[#F3F0E9]/30">
                  <div className="flex-1">
                    <input type="text" placeholder="Milestone Name (e.g. 50% Booking)" value={m.name} onChange={e => { const ms = [...milestones]; ms[i].name = e.target.value; setMilestones(ms); }} className="w-full p-2 text-sm border border-[#E8E4D9] focus:outline-none focus:border-[#0C1D42] mb-2" required />
                    <div className="flex gap-2">
                      <input type="number" placeholder="Amount (₹)" value={m.amount} onChange={e => { const ms = [...milestones]; ms[i].amount = e.target.value; setMilestones(ms); }} className="w-1/2 p-2 text-sm border border-[#E8E4D9] focus:outline-none focus:border-[#0C1D42]" required />
                      <input type="date" value={m.tentative_date} onChange={e => { const ms = [...milestones]; ms[i].tentative_date = e.target.value; setMilestones(ms); }} className="w-1/2 p-2 text-sm border border-[#E8E4D9] focus:outline-none focus:border-[#0C1D42]" required />
                    </div>
                  </div>
                  {milestones.length > 1 && (
