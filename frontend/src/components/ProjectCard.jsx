  ...
  return (
    <Link to={href} className="hs-card block overflow-hidden group" data-testid={`project-card-${project.project_id}`}>
      <div className="relative aspect-[5/3] overflow-hidden">
        ...
        {project.rera_number && (
          <span className="absolute top-3 right-3 bg-[#FAF9F6] text-[#06402B] text-[9px] tracking-widest uppercase px-2 py-1 font-semibold">
            RERA Verified
          </span>
        )}
        <div className="absolute bottom-4 left-5 right-5">
          <div className="text-[10px] tracking-widest uppercase text-[#B68D40] mb-1">
      ...
      <div className="p-5 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-1.5 text-xs text-[#4A5D54] mb-1">
            <MapPin size={12} strokeWidth={1.5} /> {project.locality}, {project.city}
          </div>
          <div className="flex items-center gap-1.5 text-xs text-[#4A5D54]">
            <Building2 size={12} strokeWidth={1.5} /> {project.unit_types}
          </div>
        </div>
        <div className="text-right">
          <div className="label-eyebrow text-[10px]">Starting</div>
          <div className="font-display text-xl text-[#06402B]">{formatINR(project.price_min)}</div>
        </div>
      </div>
    </Link>
  );
