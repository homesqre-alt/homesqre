...
export default function EmiCalculator({ initialPrice = 5000000, defaultBank = null, compact = false }) {
  ...
  return (
    <div className={`bg-white border border-[#E8E4D9] ${compact ? "p-5" : "p-8"}`}>
      <div className="flex items-baseline justify-between mb-6">
        <h3 className="font-display text-2xl text-[#06402B]" data-testid="emi-title">EMI Calculator</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          />
          <div className="text-xs text-[#4A5D54] mt-1">{formatINR(loanAmount)}</div>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="text-xs text-[#4A5D54] mt-1">{formatINR((loanAmount * downPct) / 100)}</div>
        </div>
        <div>
          ...
          <div className="text-xs text-[#4A5D54] mt-1">Rate: {rate}% p.a.</div>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-4 pt-6 border-t border-[#E8E4D9]">
        <div>
          <div className="label-eyebrow mb-1">Monthly EMI</div>
          <div className="font-display text-2xl text-[#06402B]" data-testid="emi-monthly">
            {formatINR(Math.round(emi))}
          </div>
        </div>
        <div>
          <div className="label-eyebrow mb-1">Interest</div>
          <div className="font-display text-2xl text-[#1A2421]">{formatINR(Math.round(totalInterest))}</div>
        </div>
        <div>
          <div className="label-eyebrow mb-1">Total</div>
          <div className="font-display text-2xl text-[#1A2421]">{formatINR(Math.round(totalPayment))}</div>
        </div>
      </div>
